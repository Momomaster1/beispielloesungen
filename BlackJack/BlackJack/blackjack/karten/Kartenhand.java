...
/**
 * Objekte dieser Klasse repräsentieren das Blatt eines Spielers. Es ist im
...
public class Kartenhand {
	 ...
	 * ausreichend groß angelegt werden, dass es nie Größenprobleme gibt.
	 * Gefüllt wird es aufsteigend.
	 */
	 ...
	 */
	public Kartenhand() {
		// Wir machen das Array so groß, wie es Spielkarten geben kann. Dann
		// kann nix passieren
	...
	/**
	 * Nimmt von dem übergebenen Kartenstapel die oberste Karte und fügt Sie
	...
	public void nimmKarte(Spielkartenstapel stapel) {
		...
		// Nachsehen, wo der erste freie Platz für eine Karte ist
	 ...
	 * gezählt, dass es für den Spieler das optimalste Ergebnis ergibt.
	 * Beispiele: Eine 9 und ein Ass würden als 20 Punkte gezählt werden, Zwei
	...
	public int ermittleWertOptimal() {
		// Vorgehen: wir zählen erstmal nur die Werte der Nicht-Asse und
		...
		// Jetzt wird eine Liste aller irgendwie möglichen Kartenhandwerte
		// erstellt.
		// Zuerst die Frage: Wieviele Möglichkeiten gibt es bei 1, 2, 3 oder 4
		// Assen?
		// Lösung: Bei 1 Ass kann man es mit 1 oder 11 zählen (2 Möglichkeiten)
		// Bei 2 Assen kann man keins, eins oder beide mit 1 zählen und den Rest
		// mit 11 (3 Möglichkeiten)
		// Bei 3 Assen kann man keins, eins, zwei oder alle mit 1 zählen und den
		// Rest mit 11 (4 Möglichkeiten)
		// Bei 4 Assen kann man keins, eins, zwei, drei oder alle mit 1 zählen
		// und den Rest mit 11 (5 Möglichkeiten)
		int[] listeAllerMoeglichenWerte = new int[anzahlAsse + 1];
		// Nun muss die Liste noch gefüllt werden.
		for (int i = 0; i < listeAllerMoeglichenWerte.length; i++) {
			// Für den 0. eintrag in der Liste zählen wir 0 Asse mit 1 und und
			// den Rest mit 11
			// Für den 1. eintrag in der Liste zählen wir 1 Asse mit 1 und und
			// den Rest mit 11 usw
		...
		// zählen kann.
		// Wir müssen nur noch die beste auswählen. Dabei soll noch
		// berücksichtigt werden,
		// dass z.B. 22 besser ist als 32 (zwar ist beides irgendwie nix wert,
		// doch dem Anwender vorzurechnen
		// dass {Zwei Ass Ass Acht Bube} als 32 gezählt wird ist Mist)
		...
		for (int alternativerWert : listeAllerMoeglichenWerte) {
			...
			// Gründe dafür, dass "alternativerWert" besser ist:
			// 1. Wenn der bisher beste wert > 21 ist und wir mit
			// "alternativerWert" unter 22 kommen, nehmen wir natürlich den!
			...
			// wenn er größer als "wert" ist.
			boolean beideUnter22UndAlternativerWertGroeßer = wert <= Konstanten.ZIELWERT
					&& alternativerWert <= Konstanten.ZIELWERT && alternativerWert > wert;
			// 3. Wenn beide über 21 liegen, nehmen wir "alternativerWert", wenn
			// er kleiner als "wert" ist.
			boolean beideUeber21UndAlternativerWertKleiner = wert > Konstanten.ZIELWERT
					&& alternativerWert > Konstanten.ZIELWERT && alternativerWert < wert;

			if (alternativerWertKommtUnter21 || beideUnter22UndAlternativerWertGroeßer
		...
		// Nun ist alles klar. In Wert steht die beste Zählvariante
		return wert;
	 ...
	 * mit 11 Punkten gezählt.
	 * 
	 * @return Den Punktwert der Karten auf der Hand, wobei jedes Ass 11 zählt
	 */
	...
	private int getAnzahlKartenAufHand() {
		...
		int index = 0;
		// Solange index hochzählen, wie die entsprechenden Stelle nicht leer
		// ist
		while (kartenAufHand[index] != null) {
			index++;
		}
		// Nach der Schleife hat Index den Wert der ersten leeren Stelle
		// (was gleichzeitig die Größe ist)
		return index;
	}
	...
	public String getTextdarstellung() {
		...
		// Alle weiteren evtl. vorhandenen Karten anhängen
